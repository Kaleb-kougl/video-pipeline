...
class VectorSearchManager:
    ...
    def __init__(
        self, persist_directory: str = "vector_db", collection_name: str = "anime_transcripts"
    ):
        """
    ...
    def search_episodes(
        self, query: str, limit: int = 10, show_filter: str = None, season_filter: int = None
    ) -> list[dict]:
        """
    ...
    def find_similar_episodes(
        self, show_name: str, season: int, episode: int, limit: int = 5
    ) -> list[dict]:
        """
        ...
        try:
            ...
            _episode_id = f"{show_name}_S{season}E{episode}".replace(" ", "_")
            ...
            if (
                ...
                for doc, metadata, distance in zip(
                    ...
                    if (
                        ...
                        and metadata["season"] == season
                        and metadata["episode"] == episode
                    ):
                        continue

                    episode_key = (
                        f"{metadata['show_name']}_S{metadata['season']}E{metadata['episode']}"
                    )
            ...
            similar_episodes = []
            for _episode_key, data in episode_similarities.items():
            ...
            logger.error(
                f"Failed to find similar episodes for {show_name} S{season}E{episode}: {e}"
            )
            return []
    ...
    def remove_episode(self, show_name: str, season: int, episode: int) -> bool:
        ...
        try:
            ...
            if results["ids"]:
                ...
                logger.info(
                    f"Removed {len(results['ids'])} chunks for {show_name} S{season}E{episode}"
                )
                return True
            else:
...
if __name__ == "__main__":
    ...
    if not CHROMA_AVAILABLE or not SENTENCE_TRANSFORMERS_AVAILABLE:
        ...
        success = vsm.add_episode(
            "My Hero Academia",
            1,
            4,
            sample_transcript,
            {"title": "Start Line", "quality_score": 0.85},
        )
