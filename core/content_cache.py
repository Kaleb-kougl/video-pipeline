...
class SimilarityCalculator:
    ...
    def _extract_visual_keywords(self, prompt: str) -> set:
        ...
        visual_keywords = {
            'anime', 'style', 'forest', 'village', 'training', 'battle',
            'character', 'scene', 'background', 'lighting', 'color',
            'ninja', 'magic', 'power', 'emotion', 'dramatic'
        }
...
class ContentCache:
    ...
    def __init__(self, config: Optional[CacheConfig] = None):
        ...
        logger.info(f"Content cache initialized with {self.config.max_content_entries} "
    ...
    def get_cached_content(self, 
                          content_hash: str, 
                          content_type: ContentType) -> Optional[Any]:
        """
    ...
    def find_similar_content(self, 
                           ...
                           episode_context: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        if best_match:
            logger.info(f"Found similar {content_type.value} content with "
                       f"{best_similarity:.2f} similarity")
        
        return best_match
    
    def get_or_generate_image(self, 
                             prompt: str, 
                             image_generator: Any,
    ...
    def clear_cache(self, content_type: Optional[ContentType] = None) -> None:
        """
        Clear cache entries.
    ...
    def load_from_disk(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Cache loaded from {self.config.cache_file_path} "
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'content_cache': {
                ...
                'usage_percentage': len(self._content_cache) / self.config.max_content_entries * 100
            },
            'image_cache': {
    ...
    def _generate_content_hash(self, 
                              content: Any, 
                              content_type: ContentType,
                              episode_context: Optional[str] = None) -> str:
        """
    ...
    def _evict_lru_entry(self, content_type: ContentType) -> None:
        ...
        lru_hash = min(
            cache_dict.keys(),
            key=lambda h: cache_dict[h]['last_accessed']
        )
    ...
    def _serialize_cache(self, cache_dict: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for key, value in cache_dict.items():
            ...
            for date_field in ['created_at', 'last_accessed']:
                if date_field in serialized_value and isinstance(serialized_value[date_field], datetime):
                    serialized_value[date_field] = serialized_value[date_field].isoformat()
            serialized[key] = serialized_value
        return serialized
    
    def _deserialize_cache(self, cache_data: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for key, value in cache_data.items():
            ...
            for date_field in ['created_at', 'last_accessed']:
                if date_field in deserialized_value and isinstance(deserialized_value[date_field], str):
                    try:
                        deserialized_value[date_field] = datetime.fromisoformat(deserialized_value[date_field])
                    except ValueError:
