...
def create_image(image_sentence: str, episode: str, season: str, show: str, index: int) -> None: 
    ...
    try:
        ...
        if "billed users" in error_msg or "INVALID_ARGUMENT" in error_msg:
            print(f"🖼️  Using FREE PLACEHOLDER IMAGE ({index + 1}) - Upgrade to Google Cloud billing for AI-generated anime artwork")
            logger.info(f"AI image generation requires billing - creating placeholder for: {image_sentence[:50]}...")
        elif "No Google API key found" in error_msg:
            print(f"🖼️  Using FREE PLACEHOLDER IMAGE ({index + 1}) - Add GOOGLE_API_KEY to .env for AI generation")
            logger.warning("No Google API key found - creating placeholder")
        else:
            print(f"🖼️  Using FREE PLACEHOLDER IMAGE ({index + 1}) - AI generation failed, using fallback")
...
def wave_file(show: str, season: str, episode: str, contents: str, 
              channels: int = 1, rate: int = 24000, sample_width: int = 2) -> float:
    """
    ...
    try:
        ...
        error_msg = str(e)
        if "billed users" in error_msg or "INVALID_ARGUMENT" in error_msg:
            print("🎤 Using FREE SILENT AUDIO - Upgrade to Google Cloud billing for AI-generated professional narration")
            ...
            logger.warning(f"AI audio generation failed ({error_msg[:100]}) - creating silent audio")
...
def create_silent_audio_fallback(filename: str, contents: str, sample_rate: int = 24000) -> float:
    ...
    print(f"🔇 Created {estimated_duration:.1f}s silent audio track - Perfect for adding your own voice-over!")
...
def mp4_file_enhanced(show: str, season: str, episode: str, 
                     sentences: List[str], durations: List[float]) -> None:
    """
