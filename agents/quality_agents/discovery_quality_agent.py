...
class DiscoveryQualityAgent:
    ...
    def validate_episode_discovery(
        self, discovery_data: dict, show_name: str = ""
        ...
        completeness_score = self._assess_metadata_completeness(
            discovery_data, issues, recommendations
        )

        # 2. Data accuracy validation
        accuracy_score = self._assess_data_accuracy(
            discovery_data, show_name, issues, recommendations
        )
    ...
    def _assess_metadata_completeness(
        self, discovery_data: dict, issues: list[str], recommendations: list[str]
    ) -> float:
        ...
        present_optional = sum(
            1 for field in self.optional_fields if field in discovery_data and discovery_data[field]
        )
    ...
    def _assess_data_accuracy(
        self, discovery_data: dict, show_name: str, issues: list[str], recommendations: list[str]
    ) -> float:
    ...
    def _assess_data_consistency(
        ...
        if season and episode and title:
            ...
            episode_patterns = [rf"episode\s*{episode}", rf"ep\s*{episode}", rf"e{episode}"]

            title_lower = title.lower()
            has_season_indicator = any(
                re.search(pattern, title_lower) for pattern in season_patterns
            )
            has_episode_indicator = any(
                re.search(pattern, title_lower) for pattern in episode_patterns
            )
        ...
        if url and season and episode:
            url_has_season = any(
                str(season) in url or f"s{season}" in url or f"season{season}" in url
                for season in [season]
            )
            url_has_episode = any(
                str(episode) in url or f"e{episode}" in url or f"episode{episode}" in url
    ...
    def _validate_date_format(self, date_string: str) -> bool:
        ...
        date_formats = ["%Y-%m-%d", "%Y/%m/%d", "%m/%d/%Y", "%d/%m/%Y", "%B %d, %Y", "%d %B %Y"]
    ...
    def validate_discovery_batch(self, discovery_results: list[dict], show_name: str = "") -> dict:
        ...
        reports = [
            self.validate_episode_discovery(result, show_name) for result in discovery_results
        ]
        ...
        return {
            ...
            "average_scores": {
                "metadata_completeness": sum(r.metadata_completeness for r in reports)
                / len(reports),
