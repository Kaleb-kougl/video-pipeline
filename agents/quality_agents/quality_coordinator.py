...
from .discovery_quality_agent import DiscoveryQualityAgent
from .transcript_quality_agent import TranscriptQualityAgent
from .video_quality_agent import VideoQualityAgent
from .workflow_quality_agent import WorkflowQualityAgent
...
class QualityCoordinator:
        ...
        if transcript_data:
            ...
            if transcript_report.overall_score >= min_score:
                ...
                if self.quality_gates["transcript_discovery"]["critical"]:
                    all_issues.append(
                        f"CRITICAL: Transcript quality below threshold ({transcript_report.overall_score:.2f} < {min_score})"
                    )

        # 2. Validate content generation if data provided
        if content_data:
            original_transcript = transcript_data.get("transcript", "") if transcript_data else ""
            content_report = self.content_agent.validate_ai_content(
            ...
            if content_report.overall_score >= min_score:
                ...
                if self.quality_gates["content_generation"]["critical"]:
                    all_issues.append(
                        f"CRITICAL: Content quality below threshold ({content_report.overall_score:.2f} < {min_score})"
                    )
        ...
        if video_data:
            ...
            if video_report.overall_score >= min_score:
                ...
                if self.quality_gates["video_generation"]["critical"]:
                    all_issues.append(
                        f"CRITICAL: Video quality below threshold ({video_report.overall_score:.2f} < {min_score})"
                    )

        # 4. Validate episode discovery if data provided
        if discovery_data:
            discovery_report = self.discovery_agent.validate_episode_discovery(
                discovery_data, show_name
            )
            ...
            if discovery_report.overall_score >= min_score:
                ...
                if self.quality_gates["episode_discovery"]["critical"]:
                    all_issues.append(
                        f"CRITICAL: Discovery quality below threshold ({discovery_report.overall_score:.2f} < {min_score})"
                    )
        ...
        if workflow_data:
            ...
            if workflow_report.overall_score >= min_score:
                ...
                if self.quality_gates["workflow_execution"]["critical"]:
                    all_issues.append(
                        f"CRITICAL: Workflow quality below threshold ({workflow_report.overall_score:.2f} < {min_score})"
                    )
    ...
    def get_quality_recommendations(self, unified_report: UnifiedQualityReport) -> list[str]:
        ...
        if unified_report.failed_quality_gates:
            critical_gates = [
                gate
                for gate in unified_report.failed_quality_gates
                if self.quality_gates.get(gate, {}).get("critical", False)
            ]
            if critical_gates:
                recommendations.append(
                    f"URGENT: Address critical quality gates: {', '.join(critical_gates)}"
                )
        ...
        for stage, score in sorted_stages[:3]:  # Focus on worst 3 stages
            if score < 0.7:
                if stage == "transcript_discovery":
                    ...
                    recommendations.append(
                        "Optimize video generation parameters and quality checks"
                    )
                elif stage == "episode_discovery":
                    recommendations.append(
                        "Expand episode discovery sources and metadata validation"
                    )
    ...
    def _analyze_quality_trends(self, current_scores: dict[str, float]) -> dict[str, Any]:
        ...
        for stage in current_scores.keys():
            stage_history = [
                record["stage_scores"].get(stage, 0)
        ...
        if overall_history:
            ...
            trends["overall"] = {
                "trend": "improving"
                if current_overall > overall_avg * 1.05
                else "declining"
                if current_overall < overall_avg * 0.95
                else "stable",
    ...
    def get_quality_dashboard_data(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_evaluation": self.quality_history[-1]["timestamp"].isoformat()
