...
class ContentQualityAgent:
    ...
    def validate_ai_content(
        self, content_data: dict, original_transcript: str = ""
        ...
        character_score = self._assess_character_consistency(
            content_data, original_transcript, issues, recommendations
        )
    ...
    def _assess_script_coherence(
        self, content_data: dict, issues: list[str], recommendations: list[str]
    ) -> float:
    ...
    def _assess_character_consistency(
        ...
        if original_characters:
            ...
            if missing_chars:
                score -= 0.2
                issues.append(
                    f"Missing characters from original: {', '.join(list(missing_chars)[:5])}"
                )
    ...
    def _assess_visual_descriptions(
        self, content_data: dict, issues: list[str], recommendations: list[str]
    ) -> float:
        ...
        if scenes:
            ...
            if avg_visual_density < 3:
                ...
                recommendations.append(
                    "Add more specific visual descriptions (settings, costumes, expressions)"
                )
        ...
        if len(missing_specs) > 2:
            ...
            recommendations.append(
                "Include video technical details (duration, transitions, effects)"
            )
    ...
    def _assess_technical_accuracy(
        ...
        try:
            ...
            for section_name in ["scenes", "characters", "dialogue", "visual_elements"]:
                ...
                if section is not None and not isinstance(section, (list, dict, str)):
                    ...
                    recommendations.append(
                        f"Ensure {section_name} section uses proper data structure"
                    )
        ...
        if isinstance(scenes, list):
            ...
            for i, scene in enumerate(scenes):
                if isinstance(scene, dict):
                    missing_fields = [
                        field for field in required_scene_fields if field not in scene
                    ]
                    if missing_fields:
                        score -= 0.05
    ...
    def _extract_characters(self, text: str) -> set[str]:
        ...
        characters = {
            char for char in characters if char.upper() not in false_positives and len(char) > 2
        }
    ...
    def _check_scene_flow(self, scenes: list) -> list[str]:
        ...
        for i in range(len(scenes) - 1):
            ...
            locations_current = re.findall(
                r"\b(?:room|house|school|street|park|building)\b", current_scene
            )
            locations_next = re.findall(
                r"\b(?:room|house|school|street|park|building)\b", next_scene
            )

            if (
                locations_current
                and locations_next
                and locations_current != locations_next
    ...
    def validate_content_batch(self, content_results: list[dict]) -> dict:
        ...
        return {
            ...
            "average_scores": {
                ...
                "character_consistency": sum(r.character_consistency for r in reports)
                / len(reports),
