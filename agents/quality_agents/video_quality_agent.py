...
class VideoQualityAgent:
    ...
    def validate_video_output(self, video_data: Dict, content_source: Dict = None) -> VideoQualityReport:
        """
        ...
        content_score = self._assess_content_accuracy(video_data, content_source, issues, recommendations)
        ...
        timing_score = self._assess_timing_accuracy(video_data, content_source, issues, recommendations)
    ...
    def _assess_technical_quality(self, video_data: Dict, issues: List[str], recommendations: List[str]) -> float:
        ...
        if file_ext not in self.supported_formats:
            ...
            recommendations.append(f"Convert to supported format: {', '.join(self.supported_formats)}")
        ...
        if width and height:
            if width < self.min_resolution[0] or height < self.min_resolution[1]:
                ...
                recommendations.append(f"Increase resolution to at least {self.min_resolution[0]}x{self.min_resolution[1]}")
    ...
    def _assess_content_accuracy(self, video_data: Dict, content_source: Dict, issues: List[str], recommendations: List[str]) -> float:
        ...
        if source_scenes and generated_scenes:
            ...
            if scene_diff > len(source_scenes) * 0.3:  # More than 30% difference
                score -= 0.2
                issues.append(f"Scene count mismatch: generated {len(generated_scenes)}, expected ~{len(source_scenes)}")
        ...
        if source_dialogue_count > 0:
            dialogue_ratio = video_dialogue_count / source_dialogue_count if source_dialogue_count else 0
    ...
    def _assess_visual_coherence(self, video_data: Dict, issues: List[str], recommendations: List[str]) -> float:
    ...
    def _assess_audio_quality(self, video_data: Dict, issues: List[str], recommendations: List[str]) -> float:
    ...
    def _assess_file_integrity(self, video_data: Dict, issues: List[str], recommendations: List[str]) -> float:
    ...
    def _assess_timing_accuracy(self, video_data: Dict, content_source: Dict, issues: List[str], recommendations: List[str]) -> float:
        ...
        if actual_duration and estimated_duration:
            ...
            if duration_ratio < 0.5:
                score -= 0.3
                issues.append(f"Video too short: {actual_duration}s vs expected ~{estimated_duration}s")
                recommendations.append("Increase scene duration or add more content")
            elif duration_ratio > 2.0:
                score -= 0.2
                issues.append(f"Video too long: {actual_duration}s vs expected ~{estimated_duration}s")
                recommendations.append("Reduce scene length or optimize pacing")
        
        # Check scene timing distribution
        scenes = video_data.get('scenes', [])
        if scenes:
            scene_durations = [scene.get('duration', 0) for scene in scenes if isinstance(scene, dict)]
            if scene_durations:
