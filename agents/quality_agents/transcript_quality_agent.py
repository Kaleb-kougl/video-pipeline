...
class TranscriptQualityAgent:
    ...
    def __init__(self):
        ...
        self.scene_markers = ['[', ']', '(', ')', '*', 'SCENE:', 'CUT TO:', 'FADE IN:', 'INT.', 'EXT.']
    ...
    def _assess_content_completeness(self, content: str, issues: List[str], recommendations: List[str]) -> float:
        ...
        if len(content) > self.max_content_length:
            score -= 0.2
            issues.append(f"Content unusually long: {len(content)} chars (max: {self.max_content_length})")
    ...
    def _assess_format_consistency(self, content: str, issues: List[str], recommendations: List[str]) -> float:
        """Assess the consistency of transcript formatting."""
        score = 1.0
        
        # Check for consistent dialogue formatting
        lines = content.split('\n')
        dialogue_lines = [line for line in lines if any(marker in line for marker in self.dialogue_markers)]
        ...
        scene_breaks = len([line for line in lines if any(marker in line.upper() for marker in self.scene_markers)])
    ...
    def _assess_dialogue_ratio(self, content: str, issues: List[str], recommendations: List[str]) -> float:
        """Assess the ratio of dialogue to total content."""
        lines = content.split('\n')
        dialogue_lines = len([line for line in lines if any(marker in line for marker in self.dialogue_markers)])
        total_lines = len([line for line in lines if line.strip()])
    ...
    def _assess_scene_descriptions(self, content: str, issues: List[str], recommendations: List[str]) -> float:
        ...
        action_indicators = [
            '[', ']', '(', ')',
            'walks', 'runs', 'looks', 'enters', 'exits',
            'suddenly', 'meanwhile', 'later',
    ...
    def _assess_source_reliability(self, url: str, source: str, issues: List[str], recommendations: List[str]) -> float:
