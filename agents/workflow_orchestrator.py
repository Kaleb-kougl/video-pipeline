...
class WorkflowOrchestrator:
    ...
    def __init__(self, db_path: str = "data/databases/video_generator.db"):
        ...
        self.content_agent = ContentAgent(self.model) if self.model else None  # Content analysis and generation
        ...
        self.model_with_structure = self.model.with_structured_output(Episode_Summary_Schema) if self.model else None
    ...
    def generate_structured_summary(self, content_result: Dict[str, Any], show_name: str) -> Dict[str, Any]:
        """
    ...
    async def generate_all_media(self, episode_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            character_analysis = self.character_analysis_agent.analyze_episode_characters(
                episode_data['show'], episode_data['season'], episode_data['episode'],
            ...
            logger.info(f"Enhanced episode with {len(enhanced_episode['character_focus'])} characters")
    ...
    async def process_episode_complete(self, show_name: str, season: int, episode: int, 
                               episode_title: str = None, db: DatabaseManager = None) -> ProcessingResult:
        """
        ...
        try:
            ...
            if not discovery_result or not discovery_result.get('url'):
                error_msg = f'No episode URL found for {show_name} Season {season} Episode {episode}'
            ...
            logger.info(f"Found episode URL via {discovery_result['source']} search "
            ...
            logger.info(f"Successfully parsed transcript "
    ...
    async def process_episode_from_url(self, url: str, show_name: str, db: DatabaseManager = None) -> ProcessingResult:
        """
