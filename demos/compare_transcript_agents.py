...
def compare_transcript_agents():
    ...
    try:
        from agents.enhanced_transcript_agent_fixed import (
            EnhancedTranscriptDiscoveryAgent as FixedAgent,
        )
    ...
    try:
        from agents.enhanced_transcript_agent import (
            EnhancedTranscriptDiscoveryAgent as EnhancedAgent,
        )
    ...
    for _agent_key, agent_info in agents.items():
        ...
        try:
            ...
            print(
                f"   Key Methods Available: {len(available_key_methods)}/{len(key_methods)} - {available_key_methods}"
            )
    ...
    if "transcript_agent" in agents:
        ...
        if hasattr(agent, "sources"):
            ...
            if hasattr(agent, "show_mappings"):
                ...
                print(
                    f"transcript_agent.py: {source_count} sources, {show_count} shows, {methods} methods"
                )
    ...
    if "enhanced_fixed" in agents:
        ...
        if hasattr(agent, "sources"):
            ...
            print(
                f"enhanced_transcript_agent_fixed.py: {source_count} sources, {show_count} shows, {methods} methods"
            )
...
def test_functionality():
    ...
    try:
        ...
        print("Testing transcript_agent.py with known good URL...")
        result = agent._fetch_and_parse_enhanced(
            test_url, agent.sources.get("subslikescript", {}), "subslikescript"
        )

        if result and result.get("transcript"):
            print("✅ transcript_agent.py: SUCCESS")
            ...
            print("❌ transcript_agent.py: FAILED - No transcript content")
