...
def test_chromadb_array_fixes():
    ...
    mock_results_with_data = {
        'metadatas': [[
            {'character_name': 'Test Character', 'show_name': 'Test Show', 'season': 1, 'episode': 1}
        ]],
        'documents': [['Test document']],
        'embeddings': [np.array([[0.1, 0.2, 0.3]])]  # This caused the original issue
    }
    
    mock_results_with_none = {
        'metadatas': None,
        'documents': None,
        'embeddings': None
    }
    ...
    try:
        ...
        with mock.patch('chromadb.PersistentClient'):
            ...
            results = search.semantic_search("test query", limit=5)
    ...
    try:
        ...
        with mock.patch('chromadb.PersistentClient'):
            ...
            # This should not raise array boolean errors
            quality_report = agent.check_metadata_quality()
