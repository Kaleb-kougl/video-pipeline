...
def test_character_season_analysis(tmp_path):
    ...
    try:
        with _isolated_agent_dependencies():
            ...
            assert 1 in result["episodes"] and 2 in result["episodes"], (
                "Should find episodes 1 and 2"
            )
    ...
    try:
        with _isolated_agent_dependencies():
            ...
            assert "Bakugo" in conflict_interaction["characters"], (
                "Should include Bakugo in conflict"
            )

            mentorship_interaction = ep2_interactions[0]
            assert mentorship_interaction["type"] == "mentorship", "Should identify mentorship type"
            assert mentorship_interaction["significance"] == 0.9, (
                "Should include significance score"
            )
    ...
    try:
        with _isolated_agent_dependencies():
            ...
            # Mock empty results
            empty_results = {"metadatas": [[]], "documents": [[]], "embeddings": [[]]}
