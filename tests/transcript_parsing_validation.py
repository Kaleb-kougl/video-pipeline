...
class TranscriptParsingValidator:
    ...
    def validate_current_implementation(self, html_content: str) -> dict[str, Any]:
        ...
        try:
            ...
            if transcript_element:
                ...
                results["extracted_data"]["transcript"] = (
                    transcript_text[:200] + "..." if len(transcript_text) > 200 else transcript_text
                )
    ...
    def validate_improved_implementation(self, html_content: str) -> dict[str, Any]:
        ...
        try:
                ...
                for element in elements:
                    ...
                    if quality_score > best_quality_score:
                        ...
                        results["improvements_applied"].append(
                            f"Better transcript found with selector: {selector}"
                        )

            results["extracted_data"]["transcript"] = (
                best_transcript[:200] + "..."
                if best_transcript and len(best_transcript) > 200
                else best_transcript
            )
            results["extracted_data"]["transcript_length"] = (
                len(best_transcript) if best_transcript else 0
            )
            ...
            if len(html_content) > 100000:  # Large document
                ...
                BeautifulSoup(html_content, "html.parser", parse_only=relevant_tags)
                results["improvements_applied"].append(
                    "SoupStrainer optimization for large documents"
                )
    ...
    def test_parsing_with_sample_html(self) -> None:
        ...
        if current_results.get("performance_metrics") and improved_results.get(
            "performance_metrics"
        ):
            print("\n⚡ PERFORMANCE COMPARISON:")
            ...
            if improved_time < current_time:
                ...
                print(
                    f"⚠️  Performance impact: {degradation:.1f}% slower (due to additional quality checks)"
                )
