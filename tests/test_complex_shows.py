                ...
                if "{title_slug}" not in pattern:  # Skip title-dependent patterns
                    try:
                        url = source_config["base_url"] + pattern.format(
                            show_slug=slug, season=1, episode=1
                        )
                        print(f"    {url}")
...
def test_actual_search():
    ...
    for show_name, season, episode in test_shows:
        ...
        try:
            ...
            if result:
                print("✅ Found with standard patterns!")
                ...
                print("🔍 Trying with enhanced discovery...")
                result = agent.find_episode_transcript(
                    show_name, season, episode, use_discovery=True
                )

                if result:
                    print("✅ Found with enhanced discovery!")
                    print(f"   Source: {result['source']}")
                    print(f"   Quality: {result['quality_score']:.2f}")
                    print(f"   URL: {result['url']}")
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test enhanced URL generation for complex anime show names"
    )
    parser.add_argument(
        "--mode",
