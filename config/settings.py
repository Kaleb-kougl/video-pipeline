...
class VideoConfig(BaseModel):
    ...
    default_duration_minutes: int = Field(
        default=5, description="Default video duration in minutes"
    )
    ...
    character_arcs_ratio: float = Field(
        default=0.30, description="Character arcs percentage of total"
    )
    plot_progression_ratio: float = Field(
        default=0.40, description="Plot progression percentage of total"
    )
    relationship_evolution_ratio: float = Field(
        default=0.10, description="Relationship evolution percentage"
    )
...
class Settings(BaseModel):
    ...
    database_path: str = Field(
        default="data/databases/video_generator.db", description="Path to SQLite database"
    )

    # Media settings
    output_directory: str = Field(
        default="data/episode_data", description="Output directory for generated content"
    )
    ...
    # AI Model settings
    gemini_api_key: str | None = Field(default=None, description="Gemini AI API key")
    ...
    image_model: str = Field(
        default="gemini-2.0-flash-preview-image-generation", description="Image generation model"
    )
    ...
    min_transcript_length: int = Field(
        default=500, description="Minimum transcript length for validation"
    )
    ...
    # Video configuration
    video_config: VideoConfig = Field(
        default_factory=VideoConfig, description="Video generation configuration"
    )
...
class EpisodeConfigs:
    ...
    DEFAULT_CONFIGS = {
        ...
        "Frieren: Beyond Journey's End": {"seasons": {1: {"episodes": 28, "titles": {}}}},
    }

    @classmethod
    def get_show_config(cls, show_name: str) -> dict[str, Any] | None:
        """
    ...
    @classmethod
    def get_episode_config(cls, show_name: str, season: int, episode: int) -> dict[str, Any] | None:
        """
    ...
    @classmethod
    def get_season_episodes(cls, show_name: str, season: int) -> int | None:
        """
